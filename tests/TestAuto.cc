...
namespace {
	...
	// test empty buffer
	TEST(TestZeroLengthBufferAuto_32)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 32);
		CHECK( CheckHash(cksum, "7647d9bd") );
	}

	TEST(TestZeroLengthBufferAuto_64)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 64);
		CHECK( CheckHash(cksum, "73711a77d6031b6f") );
	}

	TEST(TestZeroLengthBufferAuto_128)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 128);
		CHECK( CheckHash(cksum, "fe82e7d9998e9819c7ac954ea0a0ea8e") );
	}

	TEST(TestZeroLengthBufferAuto_256)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 256);
		CHECK( CheckHash(cksum, "c87638f19f1778d7ee03dddc5c6677fd0701e532d311bfffddfc21c0ece8910a") );
	}

	TEST(TestZeroLengthBufferAuto_512)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 512);
	...
	TEST(TestZeroLengthBufferAuto_1024)
	{
		VectorHash(buffer, 0, 0xfd4c799d, cksum, 1024);
	...
	TEST(Test128ByteBufferAuto_32)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 32);
		CHECK( CheckHash(cksum, "50c5164c") );
	}

	TEST(Test128ByteBufferAuto_64)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 64);
		CHECK( CheckHash(cksum, "a9b41194bceedd04") );
	}

	TEST(Test128ByteBufferAuto_96)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 96);
		CHECK( CheckHash(cksum, "b01e0af4bceedd04b71200b9") );
	}

	TEST(Test128ByteBufferAuto_128)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 128);
		CHECK( CheckHash(cksum, "970566856108ce7bce658104386829fa") );
	}

	TEST(Test128ByteBufferAuto_160)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 160);
		CHECK( CheckHash(cksum, "efa539560f142d7e5d1a3dc9b81f7bdf2c2b94f8") );
	}

	TEST(Test128ByteBufferAuto_192)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 192);
		CHECK( CheckHash(cksum, "6a109d9852be15a9053bb48121de06bf95006085df2f9c55") );
	}

	TEST(Test128ByteBufferAuto_224)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 224);
		CHECK( CheckHash(cksum, "ab99379bd8e63165917952a677dc830b673d34d9eecd69acbc2fcec7") );
	}

	TEST(Test128ByteBufferAuto_256)
	{
		CHECK( ReadBuffer("test0128", 128, buffer) );
		VectorHash(buffer, 128, 0xfd4c799d, cksum, 256);
		CHECK( CheckHash(cksum, "630b825fec4275e28b02590c53db6d920e3647133d5300a1e2887861867dfcf0") );
	}

	TEST(Test256ByteBufferAuto_384)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 384);
		CHECK( CheckHash(cksum, "89a883ef58dd0a9fe9ea10f66f950a9e57f9a12faaf96767a6d26a8b4d6b6bfd"
		                        "c3bd176e0ed69ebfe286c38acfad26e5") );
	}

	TEST(Test256ByteBufferAuto_512)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 512);
	...
	TEST(Test512ByteBufferAuto_768)
	{
		CHECK( ReadBuffer("test0512", 512, buffer) );
		VectorHash(buffer, 512, 0xfd4c799d, cksum, 768);
	...
	TEST(Test512ByteBufferAuto_1024)
	{
		CHECK( ReadBuffer("test0512", 512, buffer) );
		VectorHash(buffer, 512, 0xfd4c799d, cksum, 1024);
	...
	// test one blocksize
	TEST(Test256ByteBufferAuto_32)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 32);
		CHECK( CheckHash(cksum, "0eabedeb") );
	}

	TEST(Test256ByteBufferAuto_64)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 64);
		CHECK( CheckHash(cksum, "81534826f50039f6") );
	}

	TEST(Test256ByteBufferAuto_96)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 96);
		CHECK( CheckHash(cksum, "99a9d7b4f50039f60c5afc8d") );
	}

	TEST(Test256ByteBufferAuto_128)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 128);
		CHECK( CheckHash(cksum, "b384fcaef98a474cd776074b9d78bca9") );
	}

	TEST(Test256ByteBufferAuto_160)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 160);
		CHECK( CheckHash(cksum, "adf261ccc63e8066e51e6c096d32b39fcddbf78e") );
	}

	TEST(Test256ByteBufferAuto_192)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 192);
		CHECK( CheckHash(cksum, "7d3a49576f3a7d8799a7bdfeef254e39a8712c1bcbc5ad54") );
	}

	TEST(Test256ByteBufferAuto_224)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 224);
		CHECK( CheckHash(cksum, "32381fd2605a5710f0a2c49f1194dfc036e6d105e82130c1e91f57a9") );
	}

	TEST(Test256ByteBufferAuto_256)
	{
		CHECK( ReadBuffer("test0256", 256, buffer) );
		VectorHash(buffer, 256, 0xfd4c799d, cksum, 256);
		CHECK( CheckHash(cksum, "96750ccd5f66ee6fcf17f512abecd3339e487a437976ad5947589e070d8e8d9e") );
	}

	TEST(Test512ByteBufferAuto_384)
	{
		CHECK( ReadBuffer("test0512", 512, buffer) );
		VectorHash(buffer, 512, 0xfd4c799d, cksum, 384);
		CHECK( CheckHash(cksum, "778622bc016f4e7d40d4a2ad75a1b244938c9d03948560d067bf37e935c11ccd"
		                        "0ade4907afbf792a384307dc67e9d867") );
	}

	TEST(Test512ByteBufferAuto_512)
	{
		CHECK( ReadBuffer("test0512", 512, buffer) );
		VectorHash(buffer, 512, 0xfd4c799d, cksum, 512);
	...
	TEST(Test1024ByteBufferAuto_768)
	{
		CHECK( ReadBuffer("test1024", 1024, buffer) );
		VectorHash(buffer, 1024, 0xfd4c799d, cksum, 768);
	...
	TEST(Test1024ByteBufferAuto_1024)
	{
		CHECK( ReadBuffer("test1024", 1024, buffer) );
		VectorHash(buffer, 1024, 0xfd4c799d, cksum, 1024);
	...
	// test 3/2 blocksizes
	TEST(Test384ByteBufferAuto_32)
	{
		CHECK( ReadBuffer("test0384", 384, buffer) );
		VectorHash(buffer, 384, 0xfd4c799d, cksum, 32);
		CHECK( CheckHash(cksum, "7273a9bf") );
	}

	TEST(Test384ByteBufferAuto_64)
	{
		CHECK( ReadBuffer("test0384", 384, buffer) );
		VectorHash(buffer, 384, 0xfd4c799d, cksum, 64);
		CHECK( CheckHash(cksum, "e65f645584bcb59a") );
	}

	TEST(Test384ByteBufferAuto_128)
	{
		CHECK( ReadBuffer("test0384", 384, buffer) );
		VectorHash(buffer, 384, 0xfd4c799d, cksum, 128);
		CHECK( CheckHash(cksum, "00d2ad3ff87e9969156227bcedce13ea") );
	}

	TEST(Test384ByteBufferAuto_256)
	{
		CHECK( ReadBuffer("test0384", 384, buffer) );
		VectorHash(buffer, 384, 0xfd4c799d, cksum, 256);
		CHECK( CheckHash(cksum, "6838a170786ba5b21c0bce56feda9028749a294162501d660ad4ecdeee9c8245") );
	}

	TEST(Test768ByteBufferAuto_512)
	{
		CHECK( ReadBuffer("test0768", 768, buffer) );
		VectorHash(buffer, 768, 0xfd4c799d, cksum, 512);
	...
	TEST(Test1536ByteBufferAuto_1024)
	{
		CHECK( ReadBuffer("test1536", 1536, buffer) );
		VectorHash(buffer, 1536, 0xfd4c799d, cksum, 1024);
	...
	// test a large buffer
	TEST(TestLargeBufferAuto_32)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 32);
		CHECK( CheckHash(cksum, "81dddc5e") );
	}

	TEST(TestLargeBufferAuto_64)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 64);
		CHECK( CheckHash(cksum, "021fa7d3f643f834") );
	}

	TEST(TestLargeBufferAuto_128)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 128);
		CHECK( CheckHash(cksum, "5c3c9fb8481be32ea676886ab251f4fc") );
	}

	TEST(TestLargeBufferAuto_256)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 256);
		CHECK( CheckHash(cksum, "888a7ef5d0feda6a571ca947ece56c8833019cc8dca62dad01e6f9e60dcc29d3") );
	}

	TEST(TestLargeBufferAuto_512)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 512);
	...
	TEST(TestLargeBufferAuto_1024)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0xfd4c799d, cksum, 1024);
	...
	TEST(TestLargeBufferAltSeedAuto_32)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 32);
		CHECK( CheckHash(cksum, "2717e727") );
	}

	TEST(TestLargeBufferAltSeedAuto_64)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 64);
		CHECK( CheckHash(cksum, "e00c65eadc62f6d4") );
	}

	TEST(TestLargeBufferAltSeedAuto_128)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 128);
		CHECK( CheckHash(cksum, "b26873b350a8095d2ded60d9cf2d1a37") );
	}

	TEST(TestLargeBufferAltSeedAuto_256)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 256);
		CHECK( CheckHash(cksum, "79c87bd2c4d5368c277c9fc2b1b871f0ec6077b3a129a62bdcd4bee3ba44cc17") );
	}

	TEST(TestLargeBufferAltSeedAuto_512)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 512);
	...
	TEST(TestLargeBufferAltSeedAuto_1024)
	{
		CHECK( ReadBuffer("test9999", 1048576, buffer) );
		VectorHash(buffer, 1048576, 0x6ec74615, cksum, 1024);
